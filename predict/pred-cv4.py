...
model0.load_weights('weights_0_cv4.h5')

model1 = unet.get_unet()
model1.load_weights('weights_1_cv4.h5')

model2 = unet.get_unet()
model2.load_weights('weights_2_cv4.h5')

model3 = unet.get_unet()
model3.load_weights('weights_3_cv4.h5')

model4 = unet.get_unet()
model4.load_weights('weights_4_cv4.h5')


NEW=open('eva_cv4.txt','w') #change from w to a to let append --> save diff for 5 cv
for line in FILE:
    case = 'negative'
    line=line.rstrip()
    the_id=line
    ...
    if np.sum(pred_new_save)>0:
        case = 'positive'
        np.save('/media/ssd2/yananq/mace/code/pos_pred_cv4/'+the_id.split('/')[-1], pred_new)
