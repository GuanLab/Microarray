...
model0.load_weights('weights_0_cv2.h5')

model1 = unet.get_unet()
model1.load_weights('weights_1_cv2.h5')

model2 = unet.get_unet()
model2.load_weights('weights_2_cv2.h5')

model3 = unet.get_unet()
model3.load_weights('weights_3_cv2.h5')

model4 = unet.get_unet()
model4.load_weights('weights_4_cv2.h5')


NEW=open('eva_cv2.txt','w')
for line in FILE:
    case = 'negative'
    line=line.rstrip()
    the_id=line
    ...
    if np.sum(pred_new_save)>0:
        case = 'positive'
        np.save('/media/ssd2/yananq/mace/code/pos_pred_cv2/'+the_id.split('/')[-1], pred_new)
